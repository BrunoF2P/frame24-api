...
CREATE INDEX IF NOT EXISTS idx_identities_active_verified ON identity.identities(active
                                                                                            , email_verified)
       WHERE active = true ;
-- Index for blocked accounts check
CREATE INDEX IF NOT EXISTS idx_identities_blocked ON identity.identities(blocked_until)
       WHERE blocked_until IS NOT NULL ;
-- Index for employee_id lookups
CREATE INDEX IF NOT EXISTS idx_company_users_employee_company ON identity.company_users(employee_id
                                                                                                   , company_id) ;
-- Index for password history lookups (get last N passwords)
CREATE INDEX IF NOT EXISTS idx_password_history_identity_date ON identity.password_history(identity_id
                                                                                                      , created_at DESC) ;
-- Index for company users with role
CREATE INDEX IF NOT EXISTS idx_company_users_role ON identity.company_users(role_id)
       WHERE active = true ;
